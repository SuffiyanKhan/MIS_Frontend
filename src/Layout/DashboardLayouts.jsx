...
import { createTheme } from '@mui/material/styles';
import { Account, AccountPopoverFooter, AccountPreview, SignOutButton } from '@toolpad/core';
import { Avatar, Divider, IconButton, Stack, ThemeProvider } from '@mui/material';
import PropTypes from 'prop-types';
import SupportAgentIcon from '@mui/icons-material/SupportAgent';
import ChatBubbleOutlineIcon from '@mui/icons-material/ChatBubbleOutline';
import ExpandMoreIcon from '@mui/icons-material/ExpandMore';
import Cookies from 'js-cookie';
import { NAVIGATION } from '../Constant/Navigate'; // Make sure this file exists

const demoTheme = createTheme({
    palette: {
        primary: { main: '#6F2DA8', light: '#9C27B0', dark: '#3F0A78' },
        secondary: { main: '#E01B24' },
...
function SidebarFooterAccountPreview({ handleClick, open, mini }) {
    return (
        <Stack direction="column" p={0}>
            <Divider />
...
function SidebarFooterAccount({ mini }) {
    const PreviewComponent = React.useMemo(
        () => (props) => <SidebarFooterAccountPreview {...props} mini={mini} />,
        [mini]
    );

    return (
        <Box sx={{ borderTop: '2px solid #ff099b', pt: 2 }}>
            <Account
                slots={{
                    preview: PreviewComponent,
                    popoverContent: SidebarFooterAccountPopover,
...
export default function DashboardLayouts({ children }) {
        ...
        if (currentUserData) {
            const demoSession = {
                user: {
                    ...
                    image: currentUserData.profile_image,
                    role: 'SuperAdmin',
                },
            };
            setSession(demoSession);
    ...
    // 🌀 Show loader while dummy data sets up
    if (!session) {
        return (
